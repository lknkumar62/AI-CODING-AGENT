...
fun SettingsScreen(viewModel: SettingsViewModel) {
    ...
    Column(
        ...
        Text(
            "Any OpenAI-compatible endpoint works: OpenRouter, a custom API, or a local Ollama server.",
            style = MaterialTheme.typography.bodyMedium,
        )

        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedButton(onClick = { viewModel.applyPreset(AIProviderConfig.openRouterFreeCoderPreset()) }) {
                Text("Use free OpenRouter coder")
            }
            OutlinedButton(onClick = { viewModel.applyPreset(AIProviderConfig.localOllamaPreset()) }) {
        ...
        Row(
            ...
            Column {
                Text("Auto-approve safe operations", style = MaterialTheme.typography.bodyMedium)
                Text(
                    "Reads/searches only. Edits, deletes, and pushes always ask first.",
                    style = MaterialTheme.typography.labelSmall,
                )
            }
            Switch(checked = autoApprove, onCheckedChange = viewModel::setAutoApprove)
        }
